...
public class RomanToInteger {
  ...
  /*
   * 13. Roman to Integer Roman numerals are represented by seven different symbols: I, V, X, L, C,
   * D and M.
   * 
   * Symbol Value I 1 V 5 X 10 L 50 C 100 D 500 M 1000 For example, 2 is written as II in Roman
   ...
   * Example 1:
   * 
   * Input: s = "III" Output: 3 Example 2:
   * 
   * Input: s = "IV" Output: 4 Example 3:
   * 
   * Input: s = "IX" Output: 9 Example 4:
   * 
   ...
   * Constraints:
   * 
   * 1 <= s.length <= 15 s contains only the characters ('I', 'V', 'X', 'L', 'C', 'D', 'M'). It is
   * guaranteed that s is a valid roman numeral in the range [1, 3999].
   * 
   */
  public static int romanToInt(String s) {

    Map<String, Integer> prepareRomanValues = prepareRomanValues();
    int result = 0;
    if (s.length() >= 1 && s.length() <= 15) {
      if (prepareRomanValues.entrySet().stream().anyMatch(e -> s.contains(e.getKey()))) {
        for (int i = 0; i < s.length(); i++) {
          if (i + 1 != s.length()) {
            if (prepareRomanValues
                .containsKey(String.valueOf(s.charAt(i)).concat(String.valueOf(s.charAt(i + 1))))) {
              result += prepareRomanValues
                  .get(String.valueOf(s.charAt(i)).concat(String.valueOf(s.charAt(i + 1))));
              i += 1;
            } else {
              result += prepareRomanValues.get(String.valueOf(s.charAt(i)));
            }
          } else {
            result += prepareRomanValues.get(String.valueOf(s.charAt(i)));
          }
  ...
  private static Map<String, Integer> prepareRomanValues() {
